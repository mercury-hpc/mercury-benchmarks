...
# two cases for tests 1-3 - all are separated or none are.
# Means that we either have a single node alloc or >2 
# (rpc-server,bulk-server,client*)
if [[ ! ( $do_ctest1to3 == no || $num_nodes -eq 1 || $num_nodes -gt 2 ) ]] ; then
    echo "ERROR: COBALT_NODEFILE must have either 1 node or >2 to run tests 1-3" >&2
    exit 1
fi
...
mkdir -p ${out_dir_pre}4/rpcbulk.1
...
# 1 rpcbulk client
$rt -n 4 $t -b 0 -m rpcbulk $a $s $nds
if [[ $? -ne 0 ]] ; then
    echo "ERROR: test 4 (1 rpcbulk client) failed" >&2
    exit 1
fi
mv cli_send* ${out_dir_pre}4/rpcbulk.1
...
mkdir -p ${out_dir_pre}4/rpcbulk.2
...
$rt -n 4 $t -b 0 -m rpcbulk $a $s $nds
if [[ $? -ne 0 ]] ; then
    echo "ERROR: test 4 (2 rpcbulk clients) failed" >&2
    exit 1
fi
mv cli_send* ${out_dir_pre}4/rpcbulk.2
$rt -n 4 -b 1 $a $t $s $nds
if [[ $? -ne 0 ]] ; then
...
mkdir -p ${out_dir_pre}4/rpcbulk.$((num_nodes-1))
...
# test 4 - N rpcbulk clients
$rt -n 4 -b 0 -m rpcbulk $a $t $s ${nodes[@]}
if [[ $? -ne 0 ]] ; then
    echo "ERROR: test 4 ($((num_nodes-1)) rpc clients) failed" >&2
    exit 1
fi
